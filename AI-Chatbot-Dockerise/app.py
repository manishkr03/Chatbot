...
ask_date_message = """
    Oh, great, when you like to visit?
     Note: please type date in this format dd-mm-yyy
    """
...
def ask_service():
    ask_service_options = ["1) health checkup",
                            "2)  consultant",
                            "3)  medicine", 
                            "4) others"]
...
def check_service(cust_service_menu_int):
    ...
    if cust_service_menu_int != 1 and cust_service_menu_int != 2 and cust_service_menu_int != 3 and cust_service_menu_int !=4:
        cust_service_status = 0
        cust_service_response = None
        out_msg = wrong_cust_service_message
    
    if cust_service_menu_int == 1:
        cust_service_status = 1
        cust_service_response = "health checkup"
        out_msg = wrong_cust_service_message
    
    if cust_service_menu_int == 2:
        cust_service_status = 1
        cust_service_response = "consultant"
        out_msg = wrong_cust_service_message
        
    
    if cust_service_menu_int == 3:
        cust_service_status = 1
        cust_service_response = "medicine"
        out_msg = wrong_cust_service_message
        
    if cust_service_menu_int == 4:
        cust_service_status = 1
        cust_service_response = "other"
        out_msg = wrong_cust_service_message
...
def chat():
    ...
    if STATE == "SERVICE_ASKED":
        ...
        cust_service_status, cust_service_response, out_msg = check_service(cust_service_menu_int)
        
        if cust_service_status == 1:
            cust_responses["service"] = cust_service_response
            return_list_of_dicts[-1]["answer"] = cust_service_response
            return_list_of_dicts[-1]["type"] = "text"
